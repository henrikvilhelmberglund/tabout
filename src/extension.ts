...
import {
  ...
  determinePreviousSpecialCharPosition,
} from "./utils";
...
export function activate(context: vscode.ExtensionContext) {
  ...
  let tabin = vscode.commands.registerCommand("tabin", () => {
    ...
    if (characterInfo !== undefined) {
      //no tab, put selection just before the next special character
      let positionPreviousSpecialCharacter =
        determinePreviousSpecialCharPosition(
          characterInfo,
          currentLineText,
          currentPositionInLine
      ...
      if (positionPreviousSpecialCharacter > -1) {
        //Move cursor
        let previousCursorPosition = new vscode.Position(
          editor.selection.active.line,
          positionPreviousSpecialCharacter
        );
        return (editor.selection = new vscode.Selection(
